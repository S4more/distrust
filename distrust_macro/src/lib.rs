...
thread_local! {
    static FUNCTIONS: RefCell<Vec<DistributableFunction>> = RefCell::new(vec![]);
    static MIDDLEWARE: RefCell<Vec<DistributableFunction>> = RefCell::new(vec![]);
}
...
pub fn middleware(
    ...
    MIDDLEWARE.with(|v| v.borrow_mut().push(distributable_function));
...
pub fn distributable(
    ...
    FUNCTIONS.with(|v| v.borrow_mut().push(dt));
...
pub fn build(_: proc_macro::TokenStream) -> proc_macro::TokenStream {
    FUNCTIONS.with(|functions| {
        let functions = functions.borrow_mut();
        ...
        println!("{}", distributable_enum.to_token_stream());
...
fn build_redirect_function(functions: &[DistributableFunction]) -> TokenStream {
    ...
    // let middleware_part = build_middleware_function(functions);
...
// fn build_middleware_function(functions: &[DistributableFunction]) -> TokenStream {
//     let function_definitions: Vec<ItemFn> = functions
//         .iter()
